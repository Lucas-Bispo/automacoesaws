...
REPORTS = {"1": {"name": "VPC e Recursos Associados", "module": "vpc", "scope": "regional"}}
...
def find_active_vpc_regions(session):
    ...
    verificando por VPCs customizadas ou instâncias EC2 na VPC padrão.
    """
    ...
    for region_name in all_regions:
        ...
        try:
            ...
            is_active = False
            if len(vpcs) > 1 or not any(v['IsDefault'] for v in vpcs):
                is_active = True
                logging.info(f"-> Região ATIVA encontrada (VPC customizada): {region_name}")
            else:
                # Se há apenas a VPC Padrão, verifica se há instâncias EC2 nela
                default_vpc_id = next((vpc['VpcId'] for vpc in vpcs if vpc['IsDefault']), None)
                if default_vpc_id:
...
def main():
    ...
    while True:
        ...
        if choice in REPORTS:
            ...
            # (A lógica de nomes de arquivo permanece a mesma)
            ...
            try:
                ...
                # O resto do pipeline continua como antes
                ...
                data_pack = collector.collect_and_save_as_json(output_path=path_raw_json, regions_to_scan=regions_to_scan)
