...
REPORTS = {
    "1": {"name": "VPC e Recursos Associados (Relatório Sumarizado)", "module": "vpc"},
...
def find_active_vpc_regions(session):
    ...
    for region_name in all_regions:
        ...
        try:
            ...
            default_vpcs = ec2_regional.describe_vpcs(Filters=[{'Name': 'is-default', 'Values': ['true']}]).get('Vpcs', [])
            if default_vpcs:
                instances = ec2_regional.describe_instances(
                    Filters=[
                        {'Name': 'vpc-id', 'Values': [default_vpcs[0]['VpcId']]},
